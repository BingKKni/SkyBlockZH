...
## 文案风格 —— NPC 对白与技能/物品 Lore 怎么写

2026-09-10 依据一份二次元游戏(《原神》《崩坏:星穹铁道》)技能文案与 NPC 对话本地化研究定下的总纲。
借的是它们**组织信息的方法和角色声音的做法**,不借它们的机制术语(战技点、元素战技、行动提前
这类词和 SkyBlock 无关,不要出现在译文里)。以后新增的 NPC 对白和物品 Lore 都按这一节翻。

> **专名要像这个世界里本来就有的,人物对白要像人真的说出口,机制正文要像代码一样没有第二种解释。**

两类文本分工不同,风格不能互借:技能名、称号、活动名可以有诗性;机制正文必须工程化;NPC 可以说人话,
但不能把任务事实说成谜语。**不要拿对白的口吻翻技能正文,也不要拿技能说明的书面语翻 NPC 独白。**

### 动笔前先分红黄绿区

每一句先标出它属于哪一区,再决定手可以放多开:

- **红区(不可改写)**:数值、倍率、持续时间、触发频率、对象范围、伤害类型、状态/机制名、
  专有名词、任务事实、人物关系、概率与必定、先后顺序。
- **黄区(可调句法,不可改语义)**:从句位置、主动/被动、补出英文省略的主语、删掉英文为凑句子的
  重复、拆英文长句、把英文名词化改成中文动词。
- **绿区(可以再创作)**:笑话、梗、口头禅、押韵、没有机制含义的比喻、文化双关。
- **梗会升级。** 一个梗如果贯穿一条剧情线、成了成就名、角色昵称或任务回收点(陨落之星教团的
  盘子、猫、「模拟」;Tal Ker 的 Talcoins),它就从绿区升到红区,后续版本不能随手换梗。
  在 `gloss` 里注明是一次性梗还是贯穿梗。
...
- 用户于 2026-09-05 明确要求: **NPC 的发言要有人情味,避免无意译成中二宣言或正式说明书。**
  ...
  比「你渴望冒险吗?」更像面对玩家说话。
- **先问这句话在游戏里是干什么的,再问它逐字是什么意思。** 「造成伤害」在建规则;
  「你居然真的回来了?」在表现情绪;「一路顺风」可能是礼节、伏笔或双关。功能不同,翻法不同。
- **复制的是戏剧节拍,不是英文的停顿位置和信息顺序。** 英文口语常是「反应 → 主旨 → 解释」,
  中文可以按自己的语序重排,只要事实关系不变。
  `Why am I still here? Heh. Someone has to keep the lantern lit.` 不译成
  「为什么我还在这里?嘿。有人得让灯亮着。」,而是「你问我为什么还守在这儿?呵……总得有人看着这盏灯吧。」
- **一句对白 = 事实内容 + 人物态度 + 世界观锚点 + 互动钩子。** 其中任务事实和「下一步该干什么」
  是不可模糊的信息;开场白、性格化反应和笑点是最适合本土化创作的部分。
- **翻一个 NPC 之前先给它建档**,写在该 NPC 文件顶层的 `note` 里,五行就够:
  身份(职业/阵营/阶层)、与玩家的关系(陌生/尊敬/敌意/熟络)、说话水平(朴素/知识型/官僚/古雅)、
  核心性格(谨慎/话痨/讽刺/天真)、重复出现的词汇或口头禅,以及禁用风格(不说网络语/不爆粗等)。
- **角色声音不是每句话都同一种语气,而是同一组意象、措辞习惯和社会身份反复出现。**
  Tal Ker 的 stonks 梗、Bazaar 的角色梗、教团三人的模拟论都是这种「词汇场」,翻到后面几句要回头对前面。
- **口语化不等于所有 NPC 都一个性格。** 保留角色原本的情绪和身份; 剧情确实在演说、威胁或故意夸张时,
  可以保留相应语气。不要为制造亲切感硬塞网络梗、方言、称兄道弟或原文没有的人物关系。
- **称谓不做一对一替换。** `Sir / Madam / friend / buddy / mate / my lord` 按四个变量判断:
  政治或组织地位、人物关系、语境正式程度、既有译名。只是礼貌性的英文称呼,中文常可以删掉,
  用语气表亲疏;称谓本身是头衔或角色标签的一部分(Guard、Emissary、King、Professor)时,
  沿用「NPC 的职务型名字」条目里的既定译名。最常见的错误是「英文很客气 → 中文全是先生/女士/大人」,
  结果比原文更疏远。
- **古雅角色不写伪文言。** Tal Ker 的世界观独白、Royal Resident、教团例会这类庄重或戏剧化的段落,
  不靠「汝/吾/尔等/也/矣」制造古感,靠对仗、节奏和克制的用词。
- **现代或玩梗的角色要真的像现代中文使用者。** 英文梗和流行文化互文在中文里找等效笑点,
  必要时重造包袱;但事实保持等值,笑点只允许功能等值。**笑点的落点保持在句尾**,
  不要把包袱提前解释掉。
- **专名密度要控制。** 玩家第一次遇到一个概念时,一句里不连续塞多个未解释的术语;
  英文为了让玩家记住而重复的核心事实,中文可以压缩表面重复,但任务导航信息一个字都不能删。
- **语气可以放松,机制不能改。** 任务名、条件、数量、因果、概率与奖励方式仍须准确;
  「解锁奖励」不能润色成「打一次就必掉好东西」。专有名词和颜色分段仍按既有规则处理。
- **交付前把这个 NPC 的三到五句连起来读**,而不是逐句看:
  听起来是不是同一个人(不能上一句像古代官员、下一句像评论区);身份是否听得出来(学者、
  士兵、商人、孩子不该只靠名字框区分);关系有没有翻错(英文省掉的主语和称谓,中文很容易把上下级
  写成朋友);读出声顺不顺;**去掉名字和头像,玩家还能不能判断出这个人的情绪和社会位置**。
  答不上最后一问,说明只完成了语义转换,没完成角色本地化。

### 技能与物品 Lore —— 名字可以诗性,正文必须工程化

- **信息组织顺序**:优先写
  **条件 → 主体 → 动作 → 对象 → 数值 → 伤害类型/属性 → 附加效果 → 持续时间 → 再触发条件 → 上限/例外**。
  如「从目标背后发动暴击时,造成的伤害提高 X%」「每损失 X% 生命值,对亡灵生物造成的伤害 +Y%」。
  无条件效果不强加触发条件。
- **机制名和状态名一旦定名,就是数据库的 key。** 第一次出现可以花大功夫定名;第二次出现不为文采换
  同义词。Fortune 永远是时运不是幸运,Heat 永远是炎热不是高温,Pristine 纯净和 Prismatic 棱彩、
  Ferocity 凶暴和 Vicious 凶恶不能互相借用。发现有人换了同义词,把禁用的写法写进对应条目的备注。
- **译文里任何情况下都不用「」。** 状态名和机制名靠颜色分段标识,现有 GUI_Lore 近三千条译文没有一处用「」;
  原文带引号也不换成「」。颜色必须跟随对应词义和数值;利用 `order` 或带编号占位符调整语序,不给整句染成数值的颜色。
  英文空白段不能作为中文词句的颜色来源。
- **固定句式**,同类效果统一套用,不要各文件各写各的:

  | 功能 | 中文写法 | 现有样例 |
  |---|---|---|
  | 直接伤害 | 对[目标]造成 N 点伤害 | 对前方所有怪物造成 %1$s 点伤害 |
  | 倍率伤害 | 造成 N% 的[近战/技能]伤害 | 造成 %1$s% 的近战伤害 |
  | 获得属性 | 获得 +N [图标] [属性] | 获得 %2$s ✦ 速度 |
  | 提高/降低 | [属性/伤害]提高 N% / 降低 N% | 每次的法力消耗比上次提升 %2$s 倍 |
  | 施加状态 | 使目标陷入[状态] / 进入[状态] | |
  | 条件触发 | 当…时,… / 每…,… | 使用此武器击杀…时,额外掉落… |
  | 层数 | 每…获得 1 层[状态],最多叠加 N 层 | |
  | 触发频率 | 每 N 秒至多触发一次 | |
  | 持续时间 | 持续 N 秒 | 持续 %3$s 秒 |
  | 资源消耗 | 消耗 N 点法力 / 消耗硬币 | 消耗硬币发射弹射物 |
  | 上限 | 最多 N / 最多叠至初始值的 N 倍 | (最多叠至初始值的 %4$s 倍) |
  | 全套 | 全套加成: [名称] | 全套加成: 隔热护盾 |

- **动词直接用「造成 / 提高 / 降低 / 恢复 / 获得 / 施加 / 消除 / 消耗 / 传送 / 击退 / 额外掉落」**,
  英文的 `will deal / can / is able to / has the ability to` 不逐字翻成「将会造成 / 可以 / 能够 / 有能力」。
  前者是规则文本,后者是策划说明。
- **数字排版**:阿拉伯数字;`%` 紧邻数字;秒写「秒」、距离写「格」;`Nx` 倍率按「"Nx" 倍率加成」条目换算;
  占位符和汉字之间的空格按 README 的规则。中文通常比英文短,这是正常的,不要为了「显得完整」补词把句子拉长。
- **命名(技能名 / 称号 / 全套加成名)可以意象化**,允许四字或偏古典结构,如 Hydra Strike → 九头蛇之击、
  Ground Pound → 震地重击。但不建立「某个英文词永远对某个汉字」的规则——先看这件装备/这个技能的语义场,
  再定名;同一个 Strike 在不同物品上可以不同。**新技能名/标题放在 `GUI_Lore`**,
  不得借用 `GUI_Item` 的同名匹配改变物品名称。
- **交付前逐项自查**:

  | 检查项 | 要回答的问题 |
  |---|---|
  | 主体 | 到底是谁造成/获得这个效果?英文省略的主语补对了没有? |
  | 对象 | 单体、全体、附近敌人、还是自身?持有还是穿戴?每件还是全套? |
  | 条件 | 「当/若/每/期间/之后」挂在哪个效果上,作用域有没有挂错? |
  | 数值 | 百分比、固定值、倍率有没有误读?伤害系数还是倍率增量?概率还是必定?`Nx` 换算了没有? |
  | 时间 | 秒、次数、持续时间、触发间隔有没有混? |
  | 术语 | 和 `_shared/Stats.json`、本表既有条目完全一致? |
  | 层数 | 「最多 X 层」和「触发 X 次」分清了? |
  | 内置冷却 | 「每 N 秒至多触发一次」表达出来了? |
  | 伤害 | 伤害 / 力量 / 暴击伤害 / 技能伤害是四个不同的属性,没混? |
  | 状态名 | 正式状态名和普通描述没混?状态名前后一致? |
  | 限制 | 有没有为求简短删掉限制?濒死免疫有没有写成死后复活? |
...
- 核对对白时同时查看页面 DOM 的颜色样式和文本；纯文本摘要不能证明整句同色。有实测 `raw` 时优先采用实测颜色边界。
- Wiki 使用十六进制颜色时，按 RGB 距离映射到最接近的 Minecraft 16 色；保留粗体等格式，并把边界写入 `segments`。不以第一段颜色涂满整句，不从页面导航或链接默认颜色推断游戏内颜色。
- Wiki 与实测不一致时检查页面最后编辑时间；细微标点差异兼容或采用实测，近期明显不同的台词保留两种写法，无法确定语境则暂缓。
...
- 写法: `7 月 20 日`,不是 `7月20日`;带年份时写 `第 513 年 8 月 10 日`。侧边栏是屏幕上最密的一块文字,
  汉字和数字挨着写会糊成一团。年、月、日全部用阿拉伯数字,不写「第五百一十三年」「八月十日」。
- 日期里的天数用 `ordinal` 占位符(`Early Spring %s` 一条管 1st/2nd/3rd/20th 四种后缀),
  不再按后缀各写一条;月份译名见下一条。

### 空岛月份名 —— 用一月到十二月,不用季节或节气

空岛一年十二个月,英文是 `Early / (无前缀) / Late` × 春夏秋冬。中文**按月序**称呼,
以月份代替季节;此前的二十四节气写法(立春/春分/晚春……)已全部作废,不要再用。

| 原文 | 译名 | | 原文 | 译名 | | 原文 | 译名 | | 原文 | 译名 |
|---|---|---|---|---|---|---|---|---|---|---|
| Early Spring | 1 月 | | Early Summer | 4 月 | | Early Autumn | 7 月 | | Early Winter | 10 月 |
| Spring | 2 月 | | Summer | 5 月 | | Autumn | 8 月 | | Winter | 11 月 |
| Late Spring | 3 月 | | Late Summer | 6 月 | | Late Autumn | 9 月 | | Late Winter | 12 月 |

- **日期行里写阿拉伯数字**:侧边栏 `Early Autumn 20th` → `7 月 20 日`,空岛菜单 `Date: 10th Autumn 510` →
  `日期: 第 510 年 8 月 10 日`,日历标题 `Winter, Year 513` → `第 513 年 11 月`,
  Jacob 竞赛日程 `Late Summer 6th, Year 513` → `第 513 年 6 月 6 日`。`_shared/Terms.json` 的
  `skyblock_month` 词条就是这种带数字的写法,拼进 `第 %s 年 %s` 之类的模板即可。
- **台词、说明文字里单独提到某个月**写汉字月份:`next Early Autumn` → 「明年七月」;
  Sam 的 `During Summer` 随机台词说的是整个夏季(四月到六月),照常译「夏天」。
- `Spring season`(Hoppity 商店)这种指整个季节的写法仍译「春季」,必要时在 gloss 里注明是一月到三月。
- 现实公历月份(宾果的 `September 2026`)是另一套,见「公历月份名」。
...
- NPC **文件名**一律保持英文(和 `npc` 字段一致)。职务型称呼在台词、菜单、Tab 与头顶名字中都可以翻译，不能仅因它是 NPC 名牌或绿色文字就禁止翻译；人名与自造专名仍保留英文。头顶职务采用明确语料，不能按颜色猜译。
...
怪物名同样按下节的通名/专名规则处理，不因出现在头顶血条而一律保留英文。
...
- 判断顺序和物品名完全一致:这个词是不是英语里本来就有的普通词。已有确定译名须跨渲染面复用，例如 `Glacite Mutt` 在 Tab 和怪物头顶都作“极冰野狗”。翻译名字不意味着翻译玩家名、修改实体数据或把每次数值变化都采集为新文本。
...
### 2026-09-09 NEU Lore 补译的既定译名与范围

- `Ability` 作为技能标题统一译「技能」,不是「能力」。`Syphon` 沿用「注入」,`Ice Essence` 沿用「冰霜精华」。
- 新机制词:Arcane Energy → 奥术能量;Dominus → 主宰;Fervor → 热忱;Hydra Strike → 九头蛇之击;
  Hollow Armor 的 Spirit 层数 → 灵力;Homing Missiles → 追踪飞弹;Ground Pound → 震地重击;Aligned → 共调。
  它们按具体 Lore 整句使用,不写进不分语境的全局 raw 词表。
- **那一轮不新增物品名翻译,既有物品名翻译不撤销。** 正文中的既有通用资源译名照常使用;
...
| Intelligence | 法力 | | Ore Fortune | 矿石时运 |
...
- 升级历史行里的「多久以前」用 `type: time`:引擎会把 `2m` / `4h` / `23d` 换成中文
  (见 `text/Capture.java` 的 `DURATION`),一条模板管所有单位。既有紧凑时钟沿用不加空格的写法
  (`2分前`、`35天`)；活动开始/结束倒计时使用 `duration_spaced`，写作 `20 小时`、`1 天 2 小时`，不能漏掉英文单位。
...
### Garden Visitor 与花园用语

- 各地 NPC 会到访玩家的花园,提出物品委托。`Completing` / `Denying` 是交付 / 拒绝委托后的台词,不属于该 NPC 的日常职责。
- 访客对话归入 `Farming/NPC_Message`。已有同一句对话时用 `ref` 复用,不要让跨玩法的副本各自漏掉后续台词。
- 人名和品牌保留英文。`Barn` 指花园里的谷仓,不是农业岛地名 `The Barn`。

| 原文 | 统一译名 | 语境 |
|---|---|---|
| The Garden / Garden | 花园 | 玩家农业岛屿 |
| Desk | 管理台 | 花园管理入口 |
...
| Jacob's Ticket | Jacob 的票券 | 农业竞赛与 Anita 商店 |
| Personal Best | 个人最佳成绩 | 竞赛中该作物的最高收获记录 |
| Bronze / Silver / Gold medal | 铜牌 / 银牌 / 金牌 | 奖牌使用独立的 `medal` 取值,不影响同名材料 |
...
- `Taylor's Cosmetics` 沿用已有**Taylor 的装饰品**; `Taylor's Collection` 为**Taylor 的常驻系列**,
  ...
  `Summer Bundle` → **夏季礼包**。这里的季节是现实活动季节,不能套用空岛历的月份写法(Summer → 5 月)。
